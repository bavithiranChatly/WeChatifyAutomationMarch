...
public class EmailUtils
{
   ...
   {
      this(getEmailUsernameFromProperties(), getEmailPasswordFromProperties(), getEmailServerFromProperties(), emailFolder);
   }
    ...
    * 
    * @param username
    *           Email username (e.g. janedoe@email.com)
    * @param password
    *           Email password
    * @param server
   ...
   public EmailUtils(String username, String password, String server, EmailFolder emailFolder) throws MessagingException
   {
      ...
      try
      {
         System.out.println(new File("src/test/resources/email.properties").getAbsolutePath());
   ...
   /**
    * @param maxToGet
    *           maximum number of messages to get, starting from the latest. For example, enter 100 to get the last 100 messages received.
    */
    ...
    * @param unreadOnly
    *           Indicate whether to only return matched messages that are unread
    * @param maxToSearch
    *           maximum number of messages to search, starting from the latest. For example, enter 100 to search through the last 100 messages.
    */
   ...
   /**
    * Gets text from the end of a line. In this example, the subject of the email is 'Authorization Code' And the line to get the text from begins with
    * 'Authorization code:' Change these items to whatever you need for your email. This is only an example.
    */
   ...
   /**
    * Gets one line of text In this example, the subject of the email is 'Authorization Code' And the line preceding the code begins with 'Authorization code:'
   ...
   public static boolean isNotificationEmailReceivedOnGmailBySubject(String userName, String password, String subject, String description)
   {
      EmailUtils emailUtils;
      try
      {
         emailUtils = new EmailUtils(userName, password, "imap.gmail.com", EmailFolder.INBOX);
         Message[] messages = emailUtils.getMessagesBySubject(subject, true, 100);
         ...
         for (Message message : messages)
         {
            ...
            System.out.println("emailbody: "+emailUtils.getEmailBodyByMessage(message).replace("\n", " ").replace("\r", ""));

            System.out.println(message.getContent());
            if (emailUtils.getEmailBodyByMessage(message).replace("\n", " ").replace("\r", "").contains(description))
            {
               return true;
            }
         }
      ...
      {
         System.out.println("EXCEPTION OCCURED WHILE READING EMAIL FROM GMAIL ACCOUNT");
         e.printStackTrace();
         return false;
   ...
   public static String getEmailBodyByMessage(Message message)
   {
      String result = "";
      try
      {
